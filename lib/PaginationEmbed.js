...
class PaginationEmbed {
     ...
     * @param {textChannel} Eris.GuildTextableWithThread channel to send the message
     * @param {authorID} string the initiating user
     * @param {clientID} string the bot user ID
     ...
     */
    constructor(textChannel, authorID, clientID, pages = [], options = {}, components, interaction) {
    ...
    async initialize() {
        ...
        let embed;
        if (typeof this.pages[this.page - 1] === "function") {
            embed = await this.pages[this.page - 1]();
        } else {
        ...
        if (this.interaction) {
            await this.interaction.createMessage(messageContent)
            this.message = await this.interaction.getOriginalMessage()
        } else {
            this.message = await this.textChannel.createMessage(messageContent)
        }

        this.handler = new ReactionHandler.continuousReactionStream(this.message, (userID) => userID === this.authorID, false, { maxMatches: this.maxMatches, time: this.timeout });
     ...
     */
    checkPerms() {
        return this.message.channel.guild && this.message.channel.permissionsOf(this.clientID).has('manageMessages');
    }
        ...
        this.handler.on('reacted', async (event) => {
            switch (event.emoji.name) {
                case this.firstPage: {
                    if (this.advanced) {
                        if (this.checkPerms()) {
                            await this.message.removeReaction(this.firstPage, this.authorID);
                        }
                ...
                case this.back: {
                    if (this.checkPerms()) {
                        await this.message.removeReaction(this.back, this.authorID);
                    }
                ...
                case this.forth: {
                    if (this.checkPerms()) {
                        await this.message.removeReaction(this.forth, this.authorID);
                    }
                ...
                case this.lastPage: {
                    if (this.advanced) {
                        if (this.checkPerms()) {
                            await this.message.removeReaction(this.lastPage, this.authorID);
                        }
...
module.exports = {
     ...
     * @param {PaginationOptions} [options] An optional options object for overwriting defaults
     * @param {components} Array<Eris.ActionRow> components to attach to the message
     * @param {interaction} Eris.CommandInteraction get interaction message differently
     */
    createPaginationEmbed: async (textChannel, authorID, clientID, pages, options, components, interaction) => {
        const paginationEmbed = new PaginationEmbed(textChannel, authorID, clientID, pages, options, components, interaction);
