...
class PaginationEmbed {
     ...
     */
    constructor(message, pages = [], options = {}, components) {
        ...
        this.pages = pages.map((page, idx) => {
            if (!this.showPages) return page;
            const pageText = `Page ${idx + 1}/${pages.length}`;
            if (page.footer.text) {
                console.log("yoooo" + page.footer.text)
                page.footer.text += `\n${pageText}`
            } else {
                console.log("not yooooo")
                page.footer = {
                    text: pageText
                }
            }
            console.log("======================")
            console.log(page.footer.text)
            return page;
        });
    }
...
module.exports = {
     ...
     * @param {PaginationOptions} [options] An optional options object for overwriting defaults
     */
    createPaginationEmbed: async (message, pages, options, components) => {
        const paginationEmbed = new PaginationEmbed(message, pages, options, components);
